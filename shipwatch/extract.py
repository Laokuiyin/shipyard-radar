...
MILESTONE_PATTERNS = [
    ("contract", "签约/立项", r"签约|签订|承接|订单|立项"),
    ("start", "开工", r"点火开工|开工|开板|首块钢板|切割"),
    ("keel", "铺龙骨", r"铺龙骨|龙骨"),
    ("launch", "下水/出坞", r"下水|出坞"),
    ("sea_trial", "试航", r"试航"),
    ("delivery", "交付/完工", r"交付|完工"),
]
...
class RuleExtractor:
    ...
    def extract(self, title: str, content: str, yard_hint: str, published_at: date | None) -> Extraction:
        ...
        matched = [(kind, label, pattern) for kind, label, pattern in MILESTONE_PATTERNS if re.search(pattern, text)]
        yard = resolve_yard(text, self.settings, yard_hint)
        relevant = bool(yard and matched and ("船" in text or "海工" in text))
        if not relevant:
            return Extraction(relevant=False, confidence=0.75)

        title_matches = [
            (kind, label, pattern)
            for kind, label, pattern in MILESTONE_PATTERNS
            if re.search(pattern, title)
        ]
        _, progress_label, _ = (title_matches or matched)[-1]
        milestones = [
            Milestone(
                kind=kind,
                label=label,
                event_date=self._nearby_date(self._evidence(text, pattern)) or published_at,
                is_expected=bool(re.search(r"预计|计划|将于|力争", self._evidence(text, pattern))),
                evidence=self._evidence(text, pattern),
            )
            for kind, label, pattern in matched
        ]
        ...
        return Extraction(
            ...
            current_progress=progress_label,
            start_date=next((m.event_date for m in milestones if m.kind == "start"), None),
            completion_date=next((m.event_date for m in milestones if m.kind == "delivery"), None),
...
EXTRACTION_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "current_progress": {
            "type": ["string", "null"],
            "enum": ["签约/立项", "开工", "铺龙骨", "下水/出坞", "试航", "交付/完工", None],
        },
        ...
        "milestones": {
            ...
            "items": {
                ...
                "properties": {
                    "kind": {
                        "type": "string",
                        "enum": ["contract", "start", "keel", "launch", "sea_trial", "delivery"],
                    },
                    "label": {"type": "string"},
...
class LLMExtractor:
    ...
    def _prompt(self, title: str, content: str, yard_hint: str, published_at: date | None) -> str:
        ...
        return f"""
从下列官方消息提取民用新船/海工项目。允许船厂仅为：{allowed}。
...
{{
  ...
  "current_progress": "签约/立项|开工|铺龙骨|下水/出坞|试航|交付/完工|null",
  ...
  "milestones": [
    {{
      "kind": "contract|start|keel|launch|sea_trial|delivery",
      "label": "节点名称",
